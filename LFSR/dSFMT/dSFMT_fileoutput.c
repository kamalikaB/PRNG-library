...
main()
{
	int n,i,*bin,j,counter=0,index=32;
	char ch=0;
	uint32_t result,t;
	dsfmt_t dsfmt;

	FILE *fp;
    	fp = fopen("dsfmt_19650218.txt", "wb");

	dsfmt_gv_init_gen_rand(19650218);//12345
	dsfmt_init_gen_rand(&dsfmt,19650218);//12345
...
n=5000;
	...
	for(i=0;i<n;i++)
	{
		result=dsfmt_genrand_uint32(&dsfmt); // 32 bit numbers
		printf("%" PRIu32  "\n",result);
		t=result;
		//index=32;
