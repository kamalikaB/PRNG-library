/********************The program generates .bin file for Diehard and TestU01 from a seed using Tauss**********************/
...
static void taus2_set (void *vstate, unsigned long int s)
{
  taus_state_t *state = (taus_state_t *) vstate;
...
//seed = 19650218;
	...
	//fp = fopen("tauss_32_19650218.txt", "wb");

	taus_set(&temp,seed);

	for(i=0;i<n;i++)
	{
		result = taus_get(&temp);
		printf("%lu\n",result);
		t=result;
