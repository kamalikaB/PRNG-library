...
static w128_t array1[100000 / 4];

int main(int argc, char * argv[])
{
	uint64_t r64,t,se;
   	sfmt_t sfmt;
	int n,i,*bin,index=64,j,counter=0;
	//int se=9650218;//1234//seed
...
n=5000;
	FILE *fp;
    	//fp = fopen(argv[1], "wb");
fp = fopen("sfmt_64_19650218.txt", "wb");
	sfmt_init_gen_rand(&sfmt, se);

	printf("%" PRIu64  "\n",se);
	printf("%d %s\n 64 bit generated randoms\n",n, sfmt_get_idstring(&sfmt));

	for(i=0;i<n;i++)
	{
    		    /* 64 bit generation */
    		  
		r64 = sfmt_genrand_uint64(&sfmt);
		printf("%" PRIu64  "\n",r64);
		t=r64;
