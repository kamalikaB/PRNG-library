...
/********************The program generates .bin file for Diehard and TestU01 from a seed using PCG**********************/
/*************We have taken n = 1000 for lattice tests and 250 for space time diagram whereas seed values are taken from an array of 7, 1234, 12345, 19650218 and 123456789123456789*******************/
...
void main()
{
	...
	for(k=0;k<5;k++)
	{
		...
		printf("enter a output file name (with .bin) within 50 characters for seed ");
		printf("%" PRIu64  "\n",seedArray[k]);
		scanf("%s",str);
		fp = fopen(str, "wb");
		if (fp == NULL) {
