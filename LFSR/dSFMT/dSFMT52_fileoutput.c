...
main()
{
	int n,i,*bin,j,counter=0,index=52,x2;
	char ch=0;
	double result,x,x1,x3;
	dsfmt_t dsfmt;

	FILE *fp;
    	fp = fopen("dsfmt_52_123456789123456789.txt", "wb");

	dsfmt_gv_init_gen_rand(123456789123456789);//12345
	dsfmt_init_gen_rand(&dsfmt,123456789123456789);//12345
	/*printf("HOW MANY NUMBER YOU WANT TO GENERATE\n");
	scanf("%d",&n);*/
n=5000;
	for(i=0;i<n;i++)
	{
		result=dsfmt_genrand_open_open(&dsfmt); //double precesion floating point number
		//result=dsfmt_gv_genrand_open_open();
		printf("%.52lf\n",result);
		x=result;
		//index=32;
