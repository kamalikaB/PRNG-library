...
//float s7,s8,s9;
int j7=0,j8=0,j9=0;
int cnt_j7=0,cnt_j8=0,cnt_j9=0;
...
real monky_stat(char *filename, char *test, int no_tests)
{
  ...
  do{
	...
	if(test[1]=='P')
	{
		if(s7 >0.025 && s7< 0.975)
		{
			j7++; //increment no. of passed instances
		}
		cnt_j7++; //increment no. of instances
	}
	if(test[1]=='Q')
	{
		if(s7 >0.025 && s7< 0.975)
		{ 
			j8++;//increment no. of passed instances
		}
		cnt_j8++; //increment no. of instances
	}
	...
	if(test[1]=='N')
	{
		if(s7 >0.025 && s7< 0.975)
		{ 
			j9++;//increment no. of passed instances
		}
		cnt_j9++; //increment no. of instances
	}
    ...
    ++rt;
		...
		//editing
	// printf("no_tests=%d, j7=%d, cnt_j7=%d, j8=%d, cnt_j8=%d, j9=%d, cnt_j9=%d\n",no_test,j7,cnt_j7,j8,cnt_j8,j9,cnt_j9);
	...
	if(test[1]=='P')
	{
		...
		sprintf(pvalues[no_test],"%d%s%d",j7,"/",cnt_j7);
	}
	if(test[1]=='Q')
	{
		no_test++;
		...
		sprintf(pvalues[no_test],"%d%s%d",j8,"/",cnt_j8);
	}
	if(test[1]=='N')
	{
		no_test++;
		...
		sprintf(pvalues[no_test],"%d%s%d",j9,"/",cnt_j9);
	}
 	// printf("no_tests=%d\n",no_test);
  free(wds);
