...
void bitst(char *filename)
{
 ...
 /* puts("\n\t|-------------------------------------------------------------|");
  ...
  for(i=1; i<=no_obs; ++i){
   ...
   //editing
	//p_val= 1-Phi(z);
   	if( (1-Phi(z)) > 0.025 && (1-Phi(z)) < 0.975 )
   	{
      		test5++; //increment no of passed instances
   	}
	cnt_test5++; //count total no. of instances
	...
	sprintf(pvalues[no_test],"%d%s%d",test5,"/",cnt_test5);
